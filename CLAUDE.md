...
- Current version: v2.0.3 — latest published GitHub release (https://github.com/farazgoal-boop/career_copilot_premium/releases/tag/v2.0.3), tag pushed, merged to `main`. `CURRENT_APP_VERSION` in `web_app/routes.py`, `setup.py`, `career-copilot-version.txt`, and `installers/windows_setup.iss` all bumped together in commit `bf3ca170`.
- **Gap to fix next**: neither the `v2.0.1`, `v2.0.2`, nor `v2.0.3` GitHub Release has a Windows `.exe` attached (only `v2.0.0` does). No CI workflow in `.github/workflows/` builds Windows on tag push — `build-desktop-mac-linux.yml` only builds macOS + Linux, and the `v2.0.0` Windows installer was uploaded manually. Needs either a manual build+upload or a new Windows workflow before a release matches the "Client delivery" section below.
- Git tags:
