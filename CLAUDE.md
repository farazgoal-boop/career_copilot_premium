...
- Current version: v2.0.4 — tag pushed to `main` (commit `16cde82a`), but **no GitHub Release was created for it** (tag-only push, unlike v2.0.0–v2.0.3 which each got a published Release). `CURRENT_APP_VERSION` in `web_app/routes.py`, `setup.py`, `career-copilot-version.txt`, and `installers/windows_setup.iss` all bumped together in the same commit.
- **Gap to fix next**: (1) `v2.0.4` has no published GitHub Release at all yet — needs one created (with or without binaries) if the sidebar's `/api/check-update` is expected to detect it as the latest version. (2) Separately, neither the `v2.0.1`, `v2.0.2`, nor `v2.0.3` GitHub Release has a Windows `.exe` attached (only `v2.0.0` does). No CI workflow in `.github/workflows/` builds Windows on tag push — `build-desktop-mac-linux.yml` only builds macOS + Linux, and the `v2.0.0` Windows installer was uploaded manually. Needs either a manual build+upload or a new Windows workflow before a release matches the "Client delivery" section below.
- Git tags:
