...
- `.github/workflows/test-swift-overlay-macos.yml` — `workflow_dispatch`-only smoke test: builds/launches the Swift overlay on a real `macos-latest` GUI session, simulates F2/F3, screenshots the result. Re-run this after any `swift-overlay/` change.
...
Resume prompt for a fresh session: "Read CLAUDE.md. Phase 3 is complete and shipped (v2.0.0). v2.0.1 (PySide6 Mac-overlay fixes + UI polish) and v2.0.2 (native Swift/NSPanel Mac overlay, embedded in the same DMG) have since shipped on top of it, both merged to main with GitHub releases live. A CI smoke test (`test-swift-overlay-macos.yml`) has since confirmed the Swift overlay's build/launch/NSPanel show-hide/Carbon F2-F3 hotkeys genuinely work on real macOS hardware with no permission prompt — but Gatekeeper's unsigned-app warning and real fullscreen/multi-Space behavior still need a human on physical Mac hardware; that's the actual next gate, not new feature work. Phase 4 not yet defined — confirm scope with user before starting new work."
