...
Resume prompt for a fresh session: "Read CLAUDE.md. Phase 3 is complete and shipped (v2.0.0). v2.0.1 (PySide6 Mac-overlay fixes + UI polish) and v2.0.2 (native Swift/NSPanel Mac overlay, embedded in the same DMG) have since shipped on top of it, both merged to main with GitHub releases live. Neither Mac-overlay approach has been tested on real Mac hardware yet — that's the actual next gate, not new feature work. Phase 4 not yet defined — confirm scope with user before starting new work."
